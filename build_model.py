...
####################################################################################################
##### Split data to internal validation, before imputation, and training model.
####################################################################################################
from sklearn.model_selection import train_test_split

## 80/20 train/test (Internal validation)
df, df_ival, df["RECURRENCE"], df_ival["RECURRENCE"] = train_test_split(df.drop("RECURRENCE", axis=1), df["RECURRENCE"], test_size=0.2, stratify=df["RECURRENCE"], random_state=0)
...
for i in df.columns: 
    if i in categorical_variable:
        ...
        df_ival.fillna({i: df_ival[i].mode()[0]}, inplace=True)
        df[i] = df[i].astype('uint8') ### encoding to improve performance
    else: 
        df.fillna({i: df[i].median()}, inplace=True)
        df_ival.fillna({i: df_ival[i].median()}, inplace=True)
